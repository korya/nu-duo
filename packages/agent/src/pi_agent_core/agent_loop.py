...
:data:`pi_agent_core.types.AgentMessage` and only crosses into pi_ai's
LLM-compatible :class:`pi_ai.types.Message` shape at the boundary via
...
* :func:`agent_loop` / :func:`agent_loop_continue` — wrap the imperative
  drivers in an :class:`pi_ai.utils.event_stream.EventStream` for
  ``async for`` iteration.
...
* The TypeScript ``streamFn`` parameter defaults to ``streamSimple``;
  Python defaults to :func:`pi_ai.stream_simple` (resolved lazily so a
...
async def _stream_assistant_response(
    *,
    ...
    # Map pi_agent's ``"off"`` reasoning level to pi_ai's ``None`` (no
    # reasoning). pi_agent has the extra ``"off"`` literal so apps can
    # distinguish "explicitly disabled" from "not configured"; pi_ai
...
def _resolve_stream_fn(stream_fn: StreamFn | None) -> StreamFn:
    """Return ``stream_fn`` if provided, else :func:`pi_ai.stream_simple`.

    Resolved lazily so importing pi_agent_core doesn't pull pi_ai.stream
    (and its provider registrations) until the first call.
    """
    if stream_fn is not None:
        return stream_fn
    from pi_ai.stream import stream_simple  # noqa: PLC0415 — lazy import
...
async def _prepare_tool_call(
    *,
    ...
    try:
        ...
        # validate_tool_arguments expects a pi_ai Tool — wrap our AgentTool.
        from pi_ai.types import Tool as _PiAiTool  # noqa: PLC0415
...
def _to_pi_ai_tools(tools: list[AgentTool[Any, Any]] | None) -> list[Any] | None:
    """Convert :class:`AgentTool` list to pi_ai :class:`Tool` list for the LLM."""
    if not tools:
        return None
    from pi_ai.types import Tool as _PiAiTool  # noqa: PLC0415
