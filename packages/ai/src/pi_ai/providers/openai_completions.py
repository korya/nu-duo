...
:class:`pi_ai.types.OpenAICompletionsCompat` object via
...
def map_stop_reason(reason: str | None) -> StopReasonResult:
    """Map an OpenAI ``finish_reason`` to pi_ai's :data:`StopReason`."""
    if reason is None:
...
def map_reasoning_effort(
    ...
    """Map a pi_ai reasoning level via a provider-specific override table."""
...
def convert_tools(tools: list[Tool], compat: _ResolvedCompat) -> list[dict[str, Any]]:
    """Convert pi_ai :class:`Tool` list to OpenAI ``tools`` payload."""
...
def convert_messages(
    ...
    """Convert pi_ai context messages to the OpenAI ``messages`` array.
...
def stream_simple_openai_completions(
    ...
    *,
    ...
    """
    from pi_ai.types import OpenAICompletionsOptions as _OpenAIOptions  # noqa: PLC0415
