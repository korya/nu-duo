...
def convert_content_blocks(
    ...
    """Convert a pi_ai text/image content list to the Anthropic wire format.
...
def map_thinking_level_to_effort(level: ThinkingLevel | None, model_id: str) -> AnthropicEffort:
    """Map a pi_ai thinking level to an Anthropic adaptive effort level.
...
def normalize_tool_call_id(tool_call_id: str, *_args: object) -> str:
    ...
    :func:`pi_ai.providers.transform_messages.transform_messages`, which
...
def map_stop_reason(reason: str) -> StopReason:
    """Map an Anthropic stop reason to pi_ai's :data:`StopReason`."""
    if reason == "end_turn":
        return "stop"
    ...
    if reason == "stop_sequence":
        # pi_ai never supplies stop sequences, so this should not occur.
        return "stop"
    if reason == "sensitive":
        return "error"
...
def convert_tools(tools: list[Tool], *, is_oauth_token: bool) -> list[dict[str, Any]]:
    """Convert pi_ai :class:`Tool` objects to Anthropic's ``tools`` payload."""
...
def convert_messages(
    ...
    *,
    ...
    """Convert pi_ai messages to the Anthropic ``messages`` array.
...
def _handle_sdk_event(
    ...
    *,
    ...
    """Process one Anthropic SDK event, mutating ``output`` and pushing pi_ai events."""
    etype = event.type
