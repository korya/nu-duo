"""HTML page rendering for OAuth success/error callbacks.

Port of ``utils/oauth/oauth-page.ts``.
"""
...
def _render_page(
    *,
    ...
    d = html.escape(details) if details else None

    details_html = f'<div class="details">{d}</div>' if d else ""
...
def oauth_success_html(message: str) -> str:
    """Render a success page for the OAuth callback."""
    return _render_page(
...
def oauth_error_html(message: str, details: str | None = None) -> str:
    """Render an error page for the OAuth callback."""
    return _render_page(
