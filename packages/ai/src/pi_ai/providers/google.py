...
def get_thinking_level(
    ...
    """Map a pi_ai effort level to Gemini's thinking level.
...
def map_stop_reason(reason: str | None) -> StopReason:
    """Map a Gemini ``FinishReason`` to pi_ai's :data:`StopReason`.
...
def map_tool_choice(choice: str) -> str:
    """Map pi_ai tool choice to Gemini ``FunctionCallingConfigMode`` name."""
    if choice == "auto":
        return "AUTO"
    if choice == "none":
        return "NONE"
...
def convert_tools(
    ...
    *,
    ...
    """Convert pi_ai tools to Gemini function declarations.
...
def convert_messages(
    ...
    """Convert pi_ai messages to the Gemini ``contents`` array.
...
def stream_simple_google(
    ...
    *,
    ...
    Maps the pi_ai unified reasoning level to Gemini's thinking config:
    ...
    * Gemini 2.x → use ``thinking_budget`` (token budget).
    """
    from pi_ai.types import GoogleOptions as _GoogleOptions  # noqa: PLC0415
    from pi_ai.types import GoogleThinkingOptions  # noqa: PLC0415
