...
* ``Message`` union (from pi_ai) + ``CustomAgentMessages`` extension point →
...
* ``SimpleStreamOptions`` inheritance: :class:`AgentLoopConfig` is a
  ...
  pi_ai it constructs a fresh :class:`SimpleStreamOptions` from the config's
...
if TYPE_CHECKING:
    ...
    from pi_ai.types import AssistantMessageEvent, Model, ThinkingBudgets
...
# ``pi_ai.types._Model`` for their own messages — ``convert_to_llm`` is the
...
class AgentTool[TParams, TDetails]:
    ...
    Unlike :class:`pi_ai.types.Tool` this includes a UI label,
...
class AgentLoopConfig:
    """Configuration passed to :func:`pi_agent_core.agent_loop.run_agent_loop`.

    Stores raw fields rather than inheriting from
    :class:`pi_ai.types.SimpleStreamOptions` because Pydantic models don't
    ...
    # SimpleStreamOptions-style knobs, forwarded to pi_ai.stream_simple.
...
class StreamFn(Protocol):
    ...
    Defaults to :func:`pi_ai.stream_simple`; tests supply a scripted
    ...
    def __call__(
        self,
        model: Model,
        context: Any,  # pi_ai.Context, but kept loose to match stream_simple's signature
...
class AgentState(Protocol):
    """Public agent state — implemented by :class:`pi_agent_core.agent.Agent`."""
